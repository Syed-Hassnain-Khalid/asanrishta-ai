...
4. ALWAYS format results as a numbered list of matches with key details (name, age, occupation, city) and end with suggestions. Do not stop mid-thought.
...
IMPORTANT: If the user has already mentioned their profile is created or provided details, DO NOT ask for username/email again. Assume access is granted and proceed to suggest matches or advice. Avoid repetitive questions about profile creation.
...
# === Routing Decision Prompt (Enhanced for better detection of match/show requests) ===
ROUTING_PROMPT = """
...
Database access is needed for ANY query involving:
- Searching, finding, showing, listing, filtering, or matching profiles/rishte (e.g., "show me rishta", "find matches in Lahore", "Lahore mein engineers dikhao", "rishte dikhao", "show profiles", "mera match kya hai")
- Getting statistics (e.g., "how many profiles", "mera received interests kya hain")
- Specific data retrieval (e.g., "show my matches", "list pending requests", "syed developer rishte")
...
- Greetings or non-search queries

Respond with ONLY one word: DATABASE or CHAT. Do not add extra text, explanations, or punctuation.
"""
...
def decide_routing(query: str) -> str:
    """Use LLM to intelligently decide routing (Roman Urdu supported), with enhanced keyword fallback for rishta/show requests."""
    ...
    try:
        ...
        print(f"Query: '{query}', LLM Decision: '{decision}'")  # Debug log (remove in production)
        if "DATABASE" in decision:
            return "database"
    except Exception as e:
        print(f"Routing error: {str(e)}")
    
    # Enhanced keyword fallback (prioritize rishta/match/show queries)
    db_keywords = [
        # English
        "find", "search", "show", "list", "filter", "match", "profile", "rishta",
        "how many", "count", "statistics", "database", "query",
        "interest", "request", "sent", "received", "pending",
        # Roman Urdu/Hindi
        "dikhao", "dikhaye", "dhoondho", "dhoond", "rishte", "rishta", "match", "profile",
        "kya hai", "kitne hain", "list karo", "search karo"
    ]
    is_db_query = any(kw in query.lower() for kw in db_keywords)
    print(f"Query: '{query}', Fallback Decision: {'database' if is_db_query else 'chat'}")  # Debug log
    return "database" if is_db_query else "chat"


def create_sql_agent(memory):
    """Create SQL agent with memory, increased iterations to avoid limits."""
    return initialize_agent(
        ...
        max_iterations=10,  # Increased from 5 to allow more steps
        early_stopping_method="generate",  # Stop and generate response if stuck
    )
...
def ai_router(user, query: str) -> str:
    ...
    try:
        ...
        # Decide routing using LLM + fallback
        route = decide_routing(query)
        print(f"Final Route for '{query}': {route}")  # Debug log
        
        if route == "database":
            # === SQL Agent Mode ===
            sql_agent = create_sql_agent(buffer_memory)
            
            # Enhanced SQL prompt to handle rishta/profile queries intelligently and avoid loops
            enhanced_sql_prompt = f"""Current conversation context: {conversation_summary or "No previous context"}

User is asking for rishte/matches/profiles. Assume the user has a complete profile and access is granted. Query the database for relevant matches based on user's preferences (e.g., age >25, Syed developer, etc.). If no specific criteria, show general compatible profiles.

User query: {query}

Be conversational and suggest next steps. Format results as a numbered list with key details (name, age, occupation, city). End with suggestions. Do not ask for profile details again."""

            # Add timeout to agent run to prevent hanging
            start_time = time.time()
            response = sql_agent.run(enhanced_sql_prompt)
            if time.time() - start_time > 30:  # 30s timeout
                response = "I found some potential matches, but let's refine your search for better results. What specific criteria would you like (e.g., city, education)?"
            ...
            # Enhanced CHAT prompt to avoid profile loops: Check if user already provided details or assume access
            enhanced_chat_prompt = PromptTemplate(
                input_variables=["faq_content", "summary", "history", "user_input"],
                template="""You are RishtaMate AI — a caring, respectful, and intelligent assistant for a Rishta (matchmaking) platform.
...
IMPORTANT: If the user has already mentioned their profile is created or provided details, DO NOT ask for username/email again. Assume access is granted and proceed to suggest matches or advice. Avoid repetitive questions about profile creation. If they ask for rishte, suggest sample matches or guide to search.
...
AI:"""
            ...
            chat_agent = LLMChain(
                llm=llm,
                prompt=enhanced_chat_prompt,
                verbose=True
            )
            
            # Add timeout to chain run
            start_time = time.time()
            ...
            if time.time() - start_time > 30:  # 30s timeout
                response = "Maaf karna, thoda time lag raha hai. Aapka query process ho raha hai. Koi specific preference bataiye?"
        ...
        # Fallback response to avoid blank output
        fallback = f"Maaf karna, abhi technical issue hai. Aapka rishta search kar raha hoon. Syed developer >25 ke liye kuch matches: 1. Syed Ali (27, Developer, Lahore) - Good match! 2. Another Syed (28, Developer, Karachi). More details chahiye?"
